#!/bin/bash
#
# eosio-tn_roll is used to have all of the instances of the ACC daemon on a host brought down
...
if [ -z "$ACC_HOME" ]; then
    echo ACC_HOME not set - $0 unable to proceed.
    exit -1
fi
...
prog=""
RD=""
for p in eosd eosiod nodeacc; do
    prog=$p
    RD=bin
    if [ -f $RD/$prog ]; then
        break;
...
if [ \( -z "$prog" \) -o \( -z "$RD" \) ]; then
    echo unable to locate binary for eosd or eosiod or nodeacc
    exit 1
fi
...
bash $ACC_HOME/scripts/eosio-tn_down.sh

cp $SDIR/$RD/$prog $RD/$prog

if [ $DD = "all" ]; then
    for ACC_RESTART_DATA_DIR in `ls -d var/lib/node_??`; do
        bash $ACC_HOME/scripts/eosio-tn_up.sh "$*"
    done
else
    bash $ACC_HOME/scripts/eosio-tn_up.sh "$*"
fi
unset ACC_RESTART_DATA_DIR
