...
export ACC_INSTALL_DIR=${INSTALL_LOCATION:-$ACC_INSTALL_DIR}

if [[ ! -d "${ACC_INSTALL_DIR}" ]]; then
   echo "[ACC installation ${COLOR_YELLOW}NOT${COLOR_NC} found in ${ACC_INSTALL_DIR}]"
else
   # As of 1.8.0, we're using a versioned directories under home: https://github.com/ACC/eos/issues/6940
   echo "[ACC installation found: ${ACC_INSTALL_DIR}]" && INSTALL_PATHS+=("${ACC_INSTALL_DIR}") # ACC_INSTALL_DIR set in .environment
   while true; do
      [[ $NONINTERACTIVE == false ]] && read -p "Do you wish to remove the installation in ${ACC_INSTALL_DIR}? (y/n) " PROCEED
      case $PROCEED in
         "" ) echo "What would you like to do?";;
         0 | true | [Yy]* )
            ...
            echo " - ACC Removal Complete"
            break;;
         1 | false | [Nn]* ) echo " - Cancelled ACC Removal!"; exit 1;;
         * ) echo "Please type 'y' for yes or 'n' for no.";;
      esac
   done
fi

echo "[Removing ACC Dependencies]"
if [[ $ARCH == "Darwin" ]]; then
   for package in $(cat scripts/acc_build_darwin_deps | cut -d, -f1 2>/dev/null); do
      while true; do
