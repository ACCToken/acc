...
( [[ $NAME == "Ubuntu" ]] && ( [[ "$(echo ${VERSION_ID})" == "16.04" ]] || [[ "$(echo ${VERSION_ID})" == "18.04" ]] )  ) || ( echo " - You must be running 16.04.x or 18.04.x to install ACC." && exit 1 )
...
ensure-apt-packages "${REPO_ROOT}/scripts/acc_build_ubuntu_deps" $(echo ${EXTRA_DEPS[@]})
echo ""
