...
@test "${TEST_LABEL} > General" {
    ...
    [[ ! -z $(echo "${output}" | grep "Starting ACC Dependency Install") ]] || exit
    ...
    [[ ! -z $(echo "${output}" | grep "Starting ACC Build") ]] || exit
    [[ ! -z $(echo "${output}" | grep " --with-iostreams --with-date_time") ]] || exit # BOOST
    [[ ! -z $(echo "${output}" | grep "ACC has been successfully built") ]] || exit
}
