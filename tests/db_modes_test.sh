...
ACC_STUFF_DIR=$(mktemp -d)
trap "rm -rf $ACC_STUFF_DIR" EXIT
NODACC_LAUNCH_PARAMS="./programs/nodeos/nodeos -d $ACC_STUFF_DIR --config-dir $ACC_STUFF_DIR \
...
run_nodeos() {
   if (( $VERBOSE == 0 )); then
      $NODACC_LAUNCH_PARAMS --http-server-address '' --p2p-listen-endpoint '' "$@" 2>/dev/null &
   else
      $NODACC_LAUNCH_PARAMS --http-server-address '' --p2p-listen-endpoint '' "$@" &
   fi
}
...
run_expect_failure() {
   ...
   trap "kill $NODACC_PID; wait $NODACC_PID; exit 1" ALRM
   sleep 10
   if wait $NODACC_PID; then exit 1; fi
   kill $TIMER_PID
   trap ALRM
}
