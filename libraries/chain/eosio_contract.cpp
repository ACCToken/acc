...
void validate_authority_precondition( const apply_context& context, const authority& auth ) {
   for(const auto& a : auth.accounts) {
      ...
      ACC_ASSERT( acct != nullptr, action_validate_exception,
      ...
      try {
         ...
         ACC_THROW( action_validate_exception,
 ...
 */
void apply_acc_newaccount(apply_context& context) {
...
//   context.require_write_lock( config::eosio_auth_scope );
   ...
   ACC_ASSERT( validate(create.owner), action_validate_exception, "Invalid owner authority");
   ACC_ASSERT( validate(create.active), action_validate_exception, "Invalid active authority");
   ...
   if( !creator.is_privileged() ) {
      ACC_ASSERT( name_str.find( "acc." ) != 0, action_validate_exception,
                  "only privileged accounts can have names that start with 'acc.'" );
   }

   auto existing_account = db.find<account_object, by_name>(create.name);
   ACC_ASSERT(existing_account == nullptr, account_name_exists_exception,
...
void apply_acc_setcode(apply_context& context) {
   ...
   ACC_ASSERT( act.vmtype == 0, invalid_contract_vm_type, "code should be 0" );
   ACC_ASSERT( act.vmversion == 0, invalid_contract_vm_version, "version should be 0" );
   ...
   ACC_ASSERT( code_size > 0 || existing_code, set_exact_code, "contract is already cleared" );
   ...
   if( existing_code ) {
      ...
      ACC_ASSERT( old_code_entry.code_hash != code_hash, set_exact_code,
...
void apply_acc_setabi(apply_context& context) {
   auto& db  = context.db;
...
void apply_acc_updateauth(apply_context& context) {
   ...
   ACC_ASSERT(!update.permission.empty(), action_validate_exception, "Cannot create authority with empty name");
   ACC_ASSERT( update.permission.to_string().find( "acc." ) != 0, action_validate_exception,
               "Permission names that start with 'acc.' are reserved" );
   ACC_ASSERT(update.permission != update.parent, action_validate_exception, "Cannot set an authority as its own parent");
   db.get<account_object, by_name>(update.account);
   ACC_ASSERT(validate(update.auth), action_validate_exception,
              "Invalid authority: ${auth}", ("auth", update.auth));
   if( update.permission == config::active_name )
      ACC_ASSERT(update.parent == config::owner_name, action_validate_exception, "Cannot change active authority's parent from owner", ("update.parent", update.parent) );
   if (update.permission == config::owner_name)
      ACC_ASSERT(update.parent.empty(), action_validate_exception, "Cannot change owner authority's parent");
   else
      ACC_ASSERT(!update.parent.empty(), action_validate_exception, "Only owner permission can have empty parent" );

   if( update.auth.waits.size() > 0 ) {
      auto max_delay = context.control.get_global_properties().configuration.max_transaction_delay;
      ACC_ASSERT( update.auth.waits.back().wait_sec <= max_delay, action_validate_exception,
   ...
   if( permission ) {
      ACC_ASSERT(parent_id == permission->parent, action_validate_exception,
...
void apply_acc_deleteauth(apply_context& context) {
//   context.require_write_lock( config::eosio_auth_scope );
   ...
   ACC_ASSERT(remove.permission != config::active_name, action_validate_exception, "Cannot delete active authority");
   ACC_ASSERT(remove.permission != config::owner_name, action_validate_exception, "Cannot delete owner authority");
   ...
   { // Check for links to this permission
      ...
      ACC_ASSERT(range.first == range.second, action_validate_exception,
...
void apply_acc_linkauth(apply_context& context) {
//   context.require_write_lock( config::eosio_auth_scope );

   auto requirement = context.get_action().data_as<linkauth>();
   try {
      ACC_ASSERT(!requirement.requirement.empty(), action_validate_exception, "Required permission cannot be empty");
      ...
      ACC_ASSERT(account != nullptr, account_query_exception,
      ...
      ACC_ASSERT(code != nullptr, account_query_exception,
      ...
      if( requirement.requirement != config::eosio_any_name ) {
         ...
         ACC_ASSERT(permission != nullptr, permission_query_exception,
      ...
      if( link ) {
         ACC_ASSERT(link->required_permission != requirement.requirement, action_validate_exception,
...
void apply_acc_unlinkauth(apply_context& context) {
//   context.require_write_lock( config::eosio_auth_scope );
   ...
   auto link = db.find<permission_link_object, by_action_name>(link_key);
   ACC_ASSERT(link != nullptr, action_validate_exception, "Attempting to unlink authority, but no link found");
   context.add_ram_usage(
      link->account,
      -(int64_t)(config::billable_size_v<permission_link_object>)
   );
...
void apply_acc_canceldelay(apply_context& context) {
