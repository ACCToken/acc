...
void deferred_transaction_generation_context::reflector_init() {
      ...
      ACC_ASSERT( sender != account_name(), ill_formed_deferred_transaction_generation_context,
...
void transaction_header::validate()const {
   ACC_ASSERT( max_net_usage_words.value < UINT32_MAX / 8UL, transaction_exception,
...
{ try {
   ...
   for(const signature_type& sig : signatures) {
      ...
      ACC_ASSERT( sig_start < deadline, tx_cpu_usage_exceeded, "transaction signature verification executed for too long",
      ...
      ACC_ASSERT( allow_duplicate_keys || successful_insertion, tx_duplicate_sig,
...
vector<transaction_extensions> transaction::validate_and_extract_extensions()const {
   ...
   for( size_t i = 0; i < transaction_extensions.size(); ++i ) {
      ...
      ACC_ASSERT( id >= id_type_lower_bound, invalid_transaction_extension,
      ...
      ACC_ASSERT( match, invalid_transaction_extension,
                  "Transaction extension with id type ${id} is not supported",
                  ("id", id)
      );

      if( match->enforce_unique ) {
         ACC_ASSERT( i == 0 || id > id_type_lower_bound, invalid_transaction_extension,
...
uint32_t packed_transaction::get_unprunable_size()const {
   ...
   ACC_ASSERT( size <= std::numeric_limits<uint32_t>::max(), tx_too_big, "packed_transaction is too big" );
...
struct read_limiter {
   ...
   {
      ACC_ASSERT(_total + count <= Limit, tx_decompression_error, "Exceeded maximum decompressed transaction size");
      _total += count;
...
bytes packed_transaction::get_raw_transaction() const
{
   try {
      switch(compression) {
         ...
         default:
            ACC_THROW(unknown_transaction_compression, "Unknown transaction compression algorithm");
      }
...
void packed_transaction::reflector_init()
{
   ...
   ACC_ASSERT( unpacked_trx.expiration == time_point_sec(), tx_decompression_error, "packed_transaction already unpacked" );
   local_unpack_transaction({});
   local_unpack_context_free_data();
}
...
{
   try {
      ACC_ASSERT(unpacked_trx.context_free_data.empty(), tx_decompression_error, "packed_transaction.context_free_data not empty");
      switch( compression ) {
         case none:
