...
const static uint64_t system_account_name    = N(acc);
const static uint64_t null_account_name      = N(acc.null);
const static uint64_t producers_account_name = N(acc.prods);
...
const static uint64_t eosio_auth_scope       = N(acc.auth);
const static uint64_t eosio_all_scope        = N(acc.all);

const static uint64_t active_name = N(active);
const static uint64_t owner_name  = N(owner);
const static uint64_t eosio_any_name = N(acc.any);
const static uint64_t eosio_code_name = N(acc.code);
...
constexpr uint64_t ACC_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / eosio::chain::config::percent_100;
}

template<typename Number>
Number ACC_PERCENT_CEIL(Number value, uint32_t percentage) {
