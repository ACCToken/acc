...
struct dummy_action {
  ...
  ACCLIB_SERIALIZE( dummy_action, (a)(b)(c) )
};

struct u128_action {
  unsigned __int128  values[3]; //16*3

  ACCLIB_SERIALIZE( u128_action, (values) )
};
...
struct cf_action {
   ...
   ACCLIB_SERIALIZE( cf_action, (payload)(cfd_idx) )
};
...
struct dtt_action {
   ...
   ACCLIB_SERIALIZE( dtt_action, (payer)(deferred_account)(deferred_action)(permission_name)(delay_sec) )
};
...
struct invalid_access_action {
   ...
   ACCLIB_SERIALIZE( invalid_access_action, (code)(val)(index)(store) )
};
